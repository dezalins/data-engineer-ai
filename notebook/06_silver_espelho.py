...
# MAGIC
# MAGIC > **Camada silver criada como espelho do bronze com governança aplicada.**
...
# MAGIC **Notas:** 
# MAGIC
# MAGIC - A silver precisa servir várias análises, e toda linha que ela
...
# MAGIC 1. `atraso_partida_min = partida_real - partida_prevista` é subtração: silver.
# MAGIC 2. `partida_pontual = atraso <= 15` embute o número **15**, que é decisão de negócio
...
# MAGIC
# MAGIC Antes de escrever o `CAST`, medir. 
...
# MAGIC %md
# MAGIC ## 2. Criar `silver.vra` como espelho
# MAGIC
# MAGIC **Não** existe nesta query: nenhum `WHERE`, nenhum `GROUP BY`, nenhum `DISTINCT`, nenhum `JOIN`. É um `SELECT` de projeção sobre o bronze inteiro.
# MAGIC
# MAGIC As três colunas do fim: `atraso_partida_min`, `atraso_chegada_min` e `minutos_recuperados`. São subtrações entre colunas da própria linha. Não têm limiar, não classificam nada, não escondem número mágico — e, principalmente,não impedem análise nenhuma.
...
# MAGIC %md
# MAGIC ## 3. Verificação com contagem
# MAGIC
# MAGIC Este é o critério objetivo do marco. Se a diferença não for **zero**, a silver não é espelho, é recorte.
...
# MAGIC %md
# MAGIC Verificando tipagem com contagem de conversões bem-sucedidas por coluna:
...
# MAGIC %md
# MAGIC ## 4. `silver.empresas` — Dois sistemas
# MAGIC
# MAGIC Etapa que muda a forma da tabela: **unifica dois cadastros do mesmo assunto**. `bronze.empresas_nacionais` e `bronze.empresas_estrangeiras` são dois processos administrativos da ANAC descrevendo a mesma entidade de negócio — "empresa aérea que opera no Brasil".
# MAGIC
# MAGIC Isso é permitido porque **não perde informação**: a contagem da silver é a soma exata das duas, e `origem_cadastro` guarda por registro de onde ele veio. Mantem a possibilidade de voltar a olhar só as estrangeiras, consegue.
...
# MAGIC %md
# MAGIC > `GROUP BY` apenas para **conferência**, não construção. A tabela já está escrita; o agrupamento aqui só serve para olhar o 
# MAGIC > resultado, não é **materializado** na silver.
# MAGIC
# MAGIC Só 20 das 729 empresas nacionais têm código ICAO — o cadastro é dominado por aviação agrícola, táxi aéreo e aeroclube, que não têm código de três letras. Quem voa linha regular tem. Isso volta no marco-07, quando o join com o VRA for medido.
...
# MAGIC
# MAGIC Documentação não é enfeite: o consumidor final deste pipeline é um **LLM**, e o `COMMENT` é literalmente o que ele lê para decidir qual coluna usar. Coluna sem comentário é coluna que a IA vai usar errado.
# MAGIC
# MAGIC O comentário descreve **significado de negócio**, não tipo de dado. "TIMESTAMP da partida" não ajuda ninguém; "horário em que a aeronave efetivamente saiu do solo" ajuda.
...
# MAGIC %md
# MAGIC Comentário de tabela e **tags**. Tag é metadado de busca e de política: é como alguém que nunca viu este projeto encontra "todas as tabelas da camada silver" ou "tudo que é do domínio aviação" sem precisar perguntar para a gente.
...
# MAGIC
# MAGIC Comprovando a "Documentação" com`information_schema`.
...
# MAGIC
# MAGIC A silver tem quatro tabelas, todas espelho do bronze, todas documentadas, e a `vra` com exatamente a mesma contagem de linhas da origem.
# MAGIC
# MAGIC O que **não** está aqui, de propósito: `partida_pontual`, `escopo`, qualquer agregação. O limiar de 15 minutos é uma decisão do cliente — outra seguradora pode trabalhar com 30. Se ele estivesse cravado na silver, atender esse outro cliente significaria reprocessar a camada inteira. Na gold, é uma linha de SQL.
